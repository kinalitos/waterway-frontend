...
import { createContaminationReport } from "../../services/contamination-reports-api"
...
export default function NewReportPage() {
  ...
  const [images, setImages] = useState([]) // Ahora almacena objetos { file, preview }
  ...
  useEffect(() => {
    if (!userId) {
      toast.error("Debe iniciar sesión para crear un reporte.")
      navigate("/login")
    }
  }, [userId, navigate])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await createContaminationReport({
        title,
        description,
        lat: Number.parseFloat(lat),
        lng: Number.parseFloat(lng),
        created_by: userId,
        images: images.map((img) => img.file), // Enviar solo los archivos
      })
      ...
      toast.error(error.message || "No se pudo crear el reporte. Intente nuevamente.")
    } finally {
      setIsLoading(false)
    }
  }
          ...
          <CardContent className="space-y-6 pt-6">
            ...
            <div className="space-y-2">
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                ...
                {images.length < 5 && (
                  <div
                    className="flex h-32 flex-col items-center justify-center rounded-md border border-dashed p-2 hover:border-[#2ba4e0] transition-all cursor-pointer group">
                    <Label
