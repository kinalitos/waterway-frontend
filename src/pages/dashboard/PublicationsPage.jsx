      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-[#435761] max-w-2xl">
            Artículos e investigaciones sobre el Río Motagua y su conservación. Comparte conocimientos y descubrimientos
            con la comunidad.
          </p>
        </div>
        <Button
          ...
          asChild
        >
          <Link to="/dashboard/publications/new" className="flex items-center gap-2">
              ...
              publications.map((publication) => (
                ...
                className="group hover:bg-[#f8fafc] transition-colors">
                  ...
                  <TableCell className="max-w-xs">
                    <div className="line-clamp-2 text-sm text-[#435761]">{truncateText(publication.content, 120)}</div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <Avatar className="h-8 w-8 bg-[#2ba4e0]/10 text-[#2ba4e0]">
                        ...
                        />
                        <AvatarFallback>{getAuthorInitials(publication.author_name)}</AvatarFallback>
                      </Avatar>
                      <span className="text-sm">{publication.author_name || "Usuario"}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center">
                      <FileText className="mr-1.5 h-3.5 w-3.5 text-[#2ba4e0]" />
                      <span className="text-sm">{formatDate(publication.created_at)}</span>
                    </div>
                  </TableCell>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-40 border-[#418fb6]/20">
                        <DropdownMenuItem asChild className="cursor-pointer">
                          <Link to={`/dashboard/publications/${publication.id}`} className="flex items-center">
                        ...
                        {canManagePublication(publication) && (
                          <>
                            <DropdownMenuItem asChild className="cursor-pointer">
                              <Link to={`/dashboard/publications/edit/${publication.id}`} className="flex items-center">
